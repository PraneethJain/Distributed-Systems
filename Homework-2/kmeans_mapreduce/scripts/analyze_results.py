...
def analyze_and_plot(results_file):
    ...
    if not os.path.exists(results_file):
        print(f"Error: Results file '{results_file}' not found. Please run run_scaling_tests.py first.")
        return
    ...
    df['time_1_core'] = df.groupby('dataset_id')['time_seconds'].transform(lambda x: x.iloc[0])
    df['speedup'] = df['time_1_core'] / df['time_seconds']
    df['efficiency'] = df['speedup'] / df['num_cores']

    print("\n3. Data with Speedup and Efficiency Metrics:")
    print(df[['dataset_id', 'num_cores', 'time_seconds', 'speedup', 'efficiency']].head())
    ...
    if 'total_map_time_ms' in df.columns and 'total_reduce_time_ms' in df.columns:
        ...
        print(df[['dataset_id', 'num_cores', 'map_time_s', 'reduce_time_s', 'other_time_s']].head())
...
def plot_3d_surface(df):
    ...
    if not SCIPY_AVAILABLE:
        ...
        ax.scatter(df['num_points'], df['num_cores'], df['time_seconds'], c=df['time_seconds'], cmap='viridis', s=50)
...
def plot_combined_time_vs_cores(df):
    ...
    for dataset_id in sorted(df['dataset_id'].unique()):
        ...
        plt.plot(subset['num_cores'], subset['time_seconds'], marker='o', linestyle='-', label=label)
...
def plot_combined_speedup_vs_cores(df):
    ...
    for dataset_id in sorted(df['dataset_id'].unique()):
        subset = df[df['dataset_id'] == dataset_id]
        label = f"Dataset {dataset_id} ({subset['num_points'].iloc[0]} points)"
        plt.plot(subset['num_cores'], subset['speedup'], marker='o', linestyle='-', label=label)
...
def plot_combined_efficiency_vs_cores(df):
    ...
    for dataset_id in sorted(df['dataset_id'].unique()):
        subset = df[df['dataset_id'] == dataset_id]
        label = f"Dataset {dataset_id} ({subset['num_points'].iloc[0]} points)"
        plt.plot(subset['num_cores'], subset['efficiency'], marker='o', linestyle='-', label=label)
...
def plot_time_breakdown_stacked_bar(df):
    ...
    for dataset_id in datasets_to_plot:
        ...
        plt.title(f'Proportional Time Breakdown by Phase (Dataset {dataset_id}: {num_points} points)')
        plt.xticks(rotation=0)
        plt.legend(title="Phase")
        ax.yaxis.set_major_formatter(plt.FuncFormatter('{:.0%}'.format)) # Format y-axis as percentage

        plt.tight_layout()
        plt.savefig(os.path.join(PLOTS_DIR, f'proportional_breakdown_dataset_{dataset_id}.png'))
        plt.close()
